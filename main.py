import pygame
import sys
from Controller.Element.Match.match_controller import MatchController
from Controller.Element.Match.right_panel_controller import RightPanel
from Controller.UI_manager import UIManager
...
while run:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
                run = False
                break
    ui_manager.draw_match(window,match_controller.get_match(),"map_1")
    # m.handleEvent(event)
    if connector:
